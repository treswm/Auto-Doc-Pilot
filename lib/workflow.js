  ...
  try {
    ...
    while (nextPage) {
      ...
      for (const article of sectionArticles) {
        ...
        if (needsTranslation) {
          // Fetch full article content to get body for translation
          ...
          try {
            ...
            if (fullArticleRes.ok) {
              ...
              fullBody = fullArticleData.article?.body || article.body || "";
            }
          } catch (bodyErr) {
            console.warn(`   ⚠️  Could not fetch full body for article ${article.id}: ${bodyErr.message}`);
          }
...
async function translateApprovedArticles(approvalStatus, runId = null) {
  ...
  console.log(`📝 Articles to translate:`, approvalStatus.articles.map(a => `${a.id} (${a.title})`).join(", "));
  ...
  for (const article of approvalStatus.articles) {
    try {
      ...
      if (!article.body) {
        throw new Error(`Article body is empty or missing`);
      }
      ...
      if (!openaiData.choices || !openaiData.choices[0] || !openaiData.choices[0].message) {
        throw new Error(`OpenAI response missing expected fields: ${JSON.stringify(openaiData)}`);
      }

      const translatedContent = JSON.parse(openaiData.choices[0].message.content);

      // Step 3: Upload translation to Zendesk
      // Try PATCH first (for updating existing translations), fall back to POST if it fails
      ...
      // If PATCH fails with 404, the translation doesn't exist yet, so create it with POST
      if (uploadRes.status === 404) {
        console.log(`   → Translation doesn't exist yet, creating with POST...`);
      ...
      failedCount += 1;
      const errorMsg = `Article ${article.id} (${article.title}): ${err.message}`;
      console.error(`❌ Failed to translate: ${errorMsg}`);
  ...
  // Generate audit filename for later retrieval
  const timestamp = new Date().toISOString().replace(/[:.]/g, "-").slice(0, -5);
  const auditFilename = `translation_update_${timestamp}.csv`;
...
function generateAuditLog(runId, scanResults, approvalStatus, translationResults) {
  ...
  const timestamp = new Date().toISOString().replace(/[:.]/g, "-").slice(0, -5);
  const csvPath = path.join(OUTPUT_DIR, `translation_update_${timestamp}.csv`);
