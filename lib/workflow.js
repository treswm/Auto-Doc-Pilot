...
async function translateApprovedArticles(approvalStatus, runId = null) {
  ...
  for (const article of approvalStatus.articles) {
    try {
      ...
      // Step 1: Check if translation already exists
      try {
        ...
        if (checkRes.ok) {
          console.log(`⏭️  Skipping ${articleId}: translation already exists`);
          continue;
        }
      } catch (err) {
        // Translation doesn't exist, continue with translation
      }
